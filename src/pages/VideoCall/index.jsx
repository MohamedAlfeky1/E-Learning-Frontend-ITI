...
export default function VideoCall() {
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      try {
        ...
        setMyPeerId(data?.data?.myPeerId);

        console.log("My Peer ID from backend:", data?.data?.myPeerId);
        console.log("Other Peer ID from backend:", data?.data?.otherPeerId);
      } catch (error) {
  ...
  useEffect(() => {
    console.log(" Checking conditions:", {
      ...
      hasStream,
      myPeerId,
      streamReady: !!myStreamRef.current,
    });

    if (!otherPeerId || !myStreamRef.current || !myPeerId || !hasStream) {
      console.log(" Waiting for peer IDs or stream...");
      return;
    }
    ...
    const setupPeer = async () => {
      console.log(" Setting up PeerJS with ID:", myPeerId);
      console.log("Will call otherPeerId:", otherPeerId);
      ...
      peer.on("open", () => {
        if (!mounted) return;
        console.log("Peer connected successfully with ID:", myPeerId);

        if (myStreamRef.current) {
          console.log("Initiating call to:", otherPeerId);
  ...
  }, [otherPeerId, myPeerId, hasStream]);
