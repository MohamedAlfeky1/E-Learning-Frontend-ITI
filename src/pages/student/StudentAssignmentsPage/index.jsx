...
const StudentAssignmentsPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!submissionUrl && !submissionText && (!submissionAttachments || submissionAttachments.length === 0)) {
      toast.error("Please provide a URL, text, or attachment for your submission");
      return;
    }
    ...
    submitMutation.mutate(
      {
        ...
        data: {
          ...
          attachments: submissionAttachments ? Array.from(submissionAttachments) : undefined,
        },
      },
      {
        onSuccess: () => {
          ...
          {assignments.map((assignment) => {
            ...
            return (
              ...
              >
                          ...
                          {assignment.attachments.map((attachment, idx) => {
                            ...
                            return (
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-lg transition-colors border border-indigo-100"
                                download
                              >
                ...
                {myGrade &&
                  (myGrade.submissionUrl || myGrade.submissionText || (myGrade.attachments && myGrade.attachments.length > 0)) && (
                    <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                      ...
                      {myGrade.submissionUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-sm text-indigo-600 hover:underline mb-1"
                        >
                      ...
                      {myGrade.attachments && myGrade.attachments.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2 mb-1">
                          {myGrade.attachments.map((attachment, idx) => {
                            const url = getFileUrl(attachment);
                            const filename = attachment.split(/[\/\\]/).pop();
                            return (
                              <a
      ...
      <Dialog open={!!submitDialog} onOpenChange={() => setSubmitDialog(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div>
              <Label htmlFor="submissionAttachments">Attachments (optional, max 5)</Label>
              <Input
                id="submissionAttachments"
                type="file"
                multiple
                onChange={(e) => setSubmissionAttachments(e.target.files)}
                className="mt-1"
              />
            </div>
