...
const AdminTicketList = ({ selectedId, onSelect, filterStatus }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden">
      {/* Header */}
      <div className="p-5 border-b shrink-0 flex justify-between items-center bg-white z-10">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Support Tickets</h2>
          <p className="text-xs text-slate-500 mt-1">Manage and respond to student inquiries</p>
        </div>
      ...
      <ScrollArea className="flex-1 bg-slate-50/40">
        <div className="p-5 flex flex-col gap-4">
          {isLoading ? (
            [1, 2, 3, 4].map((i) => (
              <div key={i} className="h-28 bg-white border border-slate-200 animate-pulse rounded-2xl" />
            ))
          ) : tickets.length === 0 ? (
              ...
              {tickets.map((ticket) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "group relative p-4 rounded-2xl cursor-pointer transition-all duration-200 border-2 w-full shrink-0",
                      isSelected
                        ? "border-primary bg-white shadow-lg ring-4 ring-primary/5 scale-[1.01]"
                        : "border-transparent bg-white shadow-sm hover:border-slate-200 hover:shadow-md"
                    )}
                  >
                    <div className={cn(
                      "absolute left-1.5 top-1/2 -translate-y-1/2 w-1 h-10 rounded-full transition-colors",
                    ...
                    <div className="flex gap-3">
                      <div className={cn(
                        ...
                        ticket.userRole === 'teacher' ? 'bg-blue-100 text-blue-600' : 'bg-indigo-100 text-indigo-600'
                      )}>
                        {initials}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex justify-between items-center border-t pt-3 mt-1">
                          ...
                          <Badge
                            className={cn(
                              "text-[10px] h-5 px-2 font-bold uppercase tracking-tighter shadow-none",
                              ticket.status === "open" ? "bg-orange-500 hover:bg-orange-600" : "bg-slate-400"
                            )}
                          >
                            {ticket.status}
                          </Badge>
